"""ВАШ РАБОЧИЙ ФАЙЛ. Именно его вы правите в задании 3.

Это точная копия step3_defect.py — с теми же четырьмя дефектами.
Разница только в назначении:

    step3_defect.py   — образец. НЕ ТРОГАЙТЕ ЕГО.
                        Он нужен, чтобы в любой момент показать, как дефект
                        выглядел изначально, и чтобы проверки в задании 4
                        было с чем сравнивать.

    step3_student.py  — ЭТОТ ФАЙЛ. Ваша песочница. Правьте смело:
                        сломаете — скопируйте образец заново командой
                        `copy step3_defect.py step3_student.py`.

    step3_fixed.py    — эталон решения. Открывайте его только после того,
                        как сделаете задание сами. Иначе смысла не будет.

Проверить свою работу:  py -m unittest -v test_student.py

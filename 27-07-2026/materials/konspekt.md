...
# Конспект занятия 27 июля 2026 года
...
Любая программа проходит одни и те же этапы, независимо от того, называете вы их вслух или нет:

```
требования → проектирование → реализация → тестирование → выпуск → сопровождение → вывод из эксплуатации
```

Разница между методологиями — не в наборе этапов, а в том, **сколько раз** вы по ним проходите и **насколько крупными кусками**.
...
- **ГОСТ Р 56939-2024** отвечает на вопрос *что должно быть сделано и чем это подтверждается*. Это предмет проверки.
...
> **Что стоит запомнить.** В учебных курсах обычно проходят слои 1–4 и останавливаются. В РБПО именно слои 6–8 являются предметом проверки при сертификации. «Умею писать код» и «умею выпускать продукт» — разные навыки, и второй состоит в основном из слоёв, которые не проходят.
...
Проверку границ отключить настройкой сборки нельзя — она встроена в среду выполнения. Обойти её можно только явным блоком `unsafe`, который требует отдельного разрешения при компиляции. Небезопасный код в C# невозможно написать случайно — только намеренно и заметно.
...
Насколько это существенно: по данным Microsoft Security Response Center, около 70 % уязвимостей в продуктах Microsoft за 2006–2018 годы относились к нарушениям безопасности памяти — то есть к тому классу, которого в C# и Python не существует.
...
На занятии мы запускаем `ruff check step3_defect.py`, и он находит ровно одну проблему — неиспользуемую переменную. SQL-инъекцию он не видит и не должен: это не его работа.
...
Шаг, который при отрицательном результате останавливает сборку, называется **quality gate**. Без него конвейер превращается в генератор отчётов, которые никто не читает.
...
Разницей в **проверяемых свидетельствах**. Работающий код — это одно свидетельство. Остальные:
...
- тесты, среди которых есть регрессионные тесты безопасности;
- отчёт линтера;
...
Почти всё в этом списке производится автоматически, если конвейер настроен один раз. В этом и состоит смысл сдвига влево: не «делать больше работы», а «сделать так, чтобы работа делалась сама и раньше».
...
## Что будет дальше на неделе

| День | Тема | Как связано с сегодняшним |
|---|---|---|
| Вт 28.07 | Тестирование ПО, автоматизация, CI/CD, GitLab CE | Продолжение раздела 7 этого конспекта |
| Ср 29.07 | Хроника языков, декларативные языки (Prolog, SQL), сдвиг влево, фреймворки, архитектура | Продолжение разделов 3 и 5. SQL сегодня встретился как источник уязвимости, в среду разбирается как язык |
| Чт 30.07 | Конвейер РБПО: статический, композиционный, динамический анализ, фаззинг | Продолжение раздела 7 вглубь |
| Пт 31.07 | Подведение итогов, итоговое тестирование | — |

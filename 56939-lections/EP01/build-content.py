...
SOURCES = [
    dict(id='G24', title='GOST R 56939-2024 · clauses 4 and 5.1', url='https://protect.gost.ru/gost/details/f3818925-a96f-4f55-96e9-46b44720ee64', note='Requirements checked against the local final-edition DOCX/PDF; official record verifies edition dates. English clause wording is a working translation.'),
    dict(id='G16', title='Rosstandart · GOST R 56939-2016 record', url='https://protect.gost.ru/gost/details/286a588e-4a6a-4899-88f7-3c370dea1e1d', note='Approved 2016-06-01; effective 2017-06-01; replaced by the 2024 edition.'),
...
slides = []
for block in re.split(r'^## ', (ROOT / 'EP01-script-en.md').read_text(encoding='utf-8'), flags=re.M)[1:]:
...
for s in slides:
    ...
    elapsed += s['seconds']
    total += count
rows += ['', f'**Total: {total} spoken words.**', '', '## Speaking pace sensitivity', '', '| Articulation pace | Speech alone | Space left inside 30:00 for pauses/transitions |', '|---|---|---|']
for pace in [110, 115, 120, 125, 130]:
    speaking = round(total * 60 / pace)
    remaining = 1800-speaking

"""ПЗ-3: готовое решение (GREEN, 14/14).

Все пять операций реестра реализованы и проходят регрессионные тесты.
Не открывайте этот файл, пока не попробуете выполнить задание
самостоятельно в ``step3_student.py``.
"""
...
def open_read_only(path: str | Path) -> sqlite3.Connection:
    ...
    p = Path(path).resolve()
    # as_uri() экранирует ``#``, ``?`` и другие зарезервированные URI-символы.
    # Ручная сборка ``file:{p.as_posix()}`` могла открыть другой путь: SQLite
    # интерпретировал часть после ``#`` как fragment.

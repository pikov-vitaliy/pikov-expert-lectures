...
VM с KOMRAD, Windows и Linux/Astra должны видеть друг друга только внутри выделенного сегмента. Используйте `Внутреннюю сеть` VirtualBox или отдельную Host-only сеть без маршрута в LAN. Bridge запрещён; NAT временно включает преподаватель только для получения проверенных обновлений и отключает до генерации событий.

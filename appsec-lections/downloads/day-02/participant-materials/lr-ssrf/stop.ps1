...
$OwnerValue = 'appsec-day2-ssrf'
$ConfigKey = 'expert.pikov.config'
$ConfigValue = 'appsec-day2-ssrf-v1'
...
function Assert-OwnedProjectObjects {
  $specifications = @(
    [pscustomobject]@{ Kind = 'container'; List = @('ps', '--all', '--filter', "label=com.docker.compose.project=$ProjectName", '--format', '{{.ID}}'); Inspect = @('container', 'inspect'); LabelPath = 'Config' },
    [pscustomobject]@{ Kind = 'network'; List = @('network', 'ls', '--filter', "label=com.docker.compose.project=$ProjectName", '--format', '{{.ID}}'); Inspect = @('network', 'inspect'); LabelPath = 'Labels' },
    [pscustomobject]@{ Kind = 'volume'; List = @('volume', 'ls', '--filter', "label=com.docker.compose.project=$ProjectName", '--format', '{{.Name}}'); Inspect = @('volume', 'inspect'); LabelPath = 'Labels' }
  )
  foreach ($specification in $specifications) {
    foreach ($objectId in @(Invoke-Docker -Arguments $specification.List)) {
      ...
      if ($inspected.Count -ne 1) { throw "Foreign or unreadable $($specification.Kind) object '$objectId'; cleanup stopped" }
...
if ($ValidateOnly) { Write-Output 'OWNERSHIP OK'; return }
...
if ($remaining.Count -gt 0) { throw "Cleanup incomplete for '$ProjectName': $($remaining -join ', ')" }

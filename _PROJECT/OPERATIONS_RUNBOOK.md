...
- Не добавлять в `.htaccess` общий HTTP→HTTPS rewrite: этот редирект выполняется на уровне хостинга, а дублирование может создать self-redirect loop за TLS-терминатором. Допустимо только адресное правило с явной HTTPS-целью; например, legacy-редирект `RewriteRule ^$ https://pikov.expert/ru/? [R=302,L]`, защищенный regression-тестом.
...
## 8. Параллельная новая главная страница

Действующая английская главная остаётся на `/`, русская — на `/ru/`.
Верхняя текстовая ссылка ведёт на новую версию соответствующего языка.
Никаких автоматических переходов на новую версию нет.

Новые страницы находятся на `/new/`, `/new/ru/`, `/new/about/` и
`/new/ru/about/`. Английский язык является основным; переключатель открывает
соответствующую статическую страницу другого языка. Лекции открываются по
существующим адресам. До выбора новой версии основной эти четыре страницы
имеют `noindex,follow` и не добавляются в sitemap.

После изменения нового шаблона или каталога пересобрать страницы, затем
выполнить обычную цепочку из раздела 4:

```powershell
node .\_PROJECT\build-new-site.mjs
node .\_PROJECT\build-new-site.mjs --check
node .\_PROJECT\build-root-locales.mjs --check
```

Новая версия входит в существующий root ZIP `pikov.expert-root-release-*`.
В `Get-RootReleaseFiles` перечислены только четыре HTML-страницы, CSS, JS и пять
иллюстраций WebP; исходные картинки и произвольные файлы из `new/` не попадают
в публикацию. Заголовки и MIME наследуются от корневого `.htaccess`.

Публикация после прохождения exact-SHA gate разделов 5–6 затрагивает только
главный домен:

```powershell
& .\_PROJECT\deploy-hosting.ps1 -ReleaseDate $releaseDate -ExpectedSourceCommit $acceptedSha -OnlyDomains @('pikov.expert') -KeepRemoteDeployRoot
```

После публикации проверить все шесть маршрутов (`/`, `/ru/` и четыре `/new/`),
переключение языка и переходы к лекциям на компьютере и телефоне. Полный
`hosting-check.ps1` при адресной публикации продолжает проверять всю сеть;
его ошибка на другом домене не расширяет разрешённую область deployment.

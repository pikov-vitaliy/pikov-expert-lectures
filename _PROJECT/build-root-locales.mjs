...
const expected = buildRussian(fs.readFileSync(sourcePath, "utf8"));
if (mode === "--write") {
  fs.mkdirSync(path.dirname(targetPath), { recursive: true });
  fs.writeFileSync(targetPath, expected, "utf8");
  process.stdout.write(`ROOT LOCALES WRITE OK ${path.relative(rootDir, targetPath)}\n`);
} else {
  if (!fs.existsSync(targetPath)) {
    process.stderr.write(`ROOT LOCALES CHECK FAIL missing ${path.relative(rootDir, targetPath)}\n`);
    process.exit(1);
  }
  const actual = fs.readFileSync(targetPath, "utf8");
  if (actual !== expected) {
    process.stderr.write(`ROOT LOCALES CHECK FAIL stale ${path.relative(rootDir, targetPath)}\n`);
    process.exit(1);
  }
  process.stdout.write(`ROOT LOCALES CHECK OK ${path.relative(rootDir, targetPath)}\n`);
}
